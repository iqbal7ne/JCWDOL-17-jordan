...
class LHA {
  ...
  constructor(numbers: number[]) {
    ...
    for (let i = 0; i < numbers.length; i++) {
      // Fixed: changed `i <= numbers.length` to `i < numbers.length`
...
class User {
  ...
  attack(target: User): void {
    ...
    console.log(
      `${this.weapon?.name || "tangan kosong"} kena! Darah${target.health}`
    );
  }

  pick(item: Weapon | Item): void {
    //method pemilihan senjata atau penyembuhan
    if ("power" in item) {
      this.weapon = item as Weapon;
      console.log(
        `lu ambil ${this.weapon.name} dengan damage ${this.weapon.power}`
      );
    } else if ("aid" in item) {
      this.item = item as Item;
      console.log(
        `lu ambil ${this.item.name} dengan penyembuhan ${this.item.aid}`
      );
    }
  }
}
...
const pUBG = () => {
  ...
  while (player1.health > 0 && player2.health > 0) {
    ...
    if (musuh.health <= 0) {
      console.log(
        `Stop! ${
          gantianPertama ? player2 : player1
        } udah mati, jangan di serang lagi!`
      );
      break;
    }
